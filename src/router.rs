...
impl Router {
    ...
    /// Run the router loop, consuming messages from `rx` until the channel closes.
...
Any other message is forwarded to the AI backend.";
            ...
            BridgeCommand::Prompt { text } => {
                ...
                let cli_resp = executor.run(&text, &workspace_handle, &session).await?;

                // Mark session active regardless of exit code — the conversation happened.
                self.session_store.write().await.mark_active(&chat_id);

                let output = if cli_resp.exit_code != 0 {
                    let error_prefix =
                ...
                let response = formatter::format(&output, &self.output_config);
                provider.send_response(&chat_id, response).await?;
            }
        }
